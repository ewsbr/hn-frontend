<template>
  <article
    v-if="error == null"
    class="w-full flex flex-col items-center overflow-hidden text-sm md:text-base"
  >
    <Story
      :story-id="story.hnId"
      :by="story.by"
    ...
    <section
      class="flex flex-col w-full items-center sm:px-0 gap-y-4 py-4 bg-neutral-50 dark:bg-neutral-900"
    >
      <div
        class="flex flex-wrap gap-x-4 gap-y-2 px-4 sm:px-0 w-full sm:w-3/4 max-w-[688px] xl:1/2"
      >
      ...
      <div
        class="flex justify-center sm:w-3/4 max-w-[688px] overflow-hidden"
      >
        <div class="px-4 sm:px-0 w-full">
          <Comments
            v-if="story.kids.length > 0"
            :story-id="story.hnId"
            :depth="0"
            :comments="story.kids"
          ></Comments>
